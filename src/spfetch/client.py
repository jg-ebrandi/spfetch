...
class SharePointClient:
    ...
    async def _get_site_id(
        self, http_client: httpx.AsyncClient, hostname: str, site_path: str
    ) -> str:
    ...
    async def _get_file_size(
        self, http_client: httpx.AsyncClient, site_id: str, file_path: str
    ) -> int:
    ...
    async def download(
        ...
        files_src = (
            [f.strip() for f in file_path.split("|")]
            if isinstance(file_path, str)
            else file_path
        )
        files_dst = (
            [d.strip() for d in dest_path.split("|")]
        ...
        if len(files_src) != len(files_dst):
            raise ValueError(
                "O número de arquivos de origem deve ser igual ao de destino."
            )
                    ...
                    try:
                        ...
                        async with httpx.AsyncClient(
                            ...
                            site_id = await self._get_site_id(
                                http_client, hostname, site_path
                            )
                            total_size = await self._get_file_size(
                                http_client, site_id, src
                            )
                            ...
                            async with http_client.stream(
                                ...
                                p1 = tqdm(
                                    ...
                                    unit="B",
                                    unit_scale=True,
                                    desc=f"📥 Reading | Leitura  ({short_name})",
                                    colour="#3498db",
                                ...
                                p2 = tqdm(
                                    ...
                                    unit="B",
                                    unit_scale=True,
                                    desc=f"📤 Saving  | Salvando   ({short_name})",
                                    colour="#2ecc71",
                                    position=pos_offset + 2,
                                    leave=False,
                                )
                        ...
                        avg_speed = (
                            (total_size / (1024**2)) / duration if duration > 0 else 0
                        )
...
-------------------------------------------------------\n"""
                        ...
                        if attempt < max_attempts:
                            ...
                            warn_p1 = tqdm(
                                total=0,
                                bar_format="{desc}",
                                desc=f"⚠️ FALHA | RETRY ({short_name}) [{attempt}/{max_attempts}]",
                                position=pos_offset + 1,
                                leave=False,
                            )
                            warn_p2 = tqdm(
                                total=0,
                                bar_format="{desc}",
                                desc=f"⏳ Aguardando 3s... Erro: {str(e)[:45]}",
                                position=pos_offset + 2,
                                leave=False,
                            )
                            ...
                            await asyncio.sleep(
                                3
                            )  # Dorme os 3s com as mensagens ativas na tela
...
-------------------------------------------------------\n"""
    ...
    async def read_df(
        ...
        **kwargs,
        ...
        try:
            ...
            raise ImportError(
                "Pandas não encontrado. Instale com: pip install spfetch[pandas]"
            )
    ...
    async def ls(self, hostname: str, site_path: str, folder_path: str = "/") -> list:
        async with httpx.AsyncClient(follow_redirects=True) as http_client:
            ...
            endpoint = (
                f"{self.base_graph_url}/sites/{site_id}/drive/root:/{quote(clean_folder, safe='/')}:/children"
                if clean_folder
                else f"{self.base_graph_url}/sites/{site_id}/drive/root/children"
            )
